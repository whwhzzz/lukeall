...
public class Luke extends Thinlet implements ClipboardOwner {
  ...
  private void initOverview() {
    try {
      ...
      String divText = "N/A";
      if (ir instanceof DirectoryReader) {
    	List<AtomicReaderContext> readerContexts = ((DirectoryReader)ir).leaves();
        List<AtomicReader> readers = new LinkedList<AtomicReader>();
        for(AtomicReaderContext c : readerContexts) {
        	readers.add(c.reader());
        }
...
//    add(r1, c1);
  ...
  public void actionExamineNorm(Object table) throws Exception {
    ...
    if (ar != null) {
     try {
       ...
       long curFVal = norms.get(docNum.intValue());
  ...
  public void showFirstTermDoc(final Object fText) {
    ...
    SlowThread st = new SlowThread(this) {
      public void execute() {
        try {
          DocsEnum td = ar.termDocsEnum(new Term(t.field(), new BytesRef(t.text())));
          if (td == null) {
        ...
        try {
          ...
          for (int i = 0; i < freq; i++) {
            try {
              ...
              cell = create("cell");
              add(r, cell);
              if (td.getPayload() != null && td.getPayload().length > 0) {
  ...
  public static Luke startLuke(String[] args) {
    Luke luke = new Luke();
    FrameLauncher f = new FrameLauncher("Luke - Lucene Index Toolbox, v 4.3_BETA (2013-07-03)", luke, 850, 650);
